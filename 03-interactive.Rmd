...
For a few years now, you could use Javascript to create interactive plots to include on 
...
Irma's Florida landfall, September 10, 2017.]
...
```{r dt_example, caption = "Example of an interactive data table. Each observation (row) is the information for one of the fatal motor vehicle accidents in Florida the week of Hurricane Irma's landfall, with columns for the county where the accident occurred, the date of the accident, and the number of fatalities."}
library(DT)
fl_accidents %>% 
  datatable()
```
...
You can do a lot of customizations through the `datatable` call. For example, you can 
add a caption to the table, change the appearance using the `class` parameter, and 
use clearer names for the columns. Try the next call to see an example of how these
...
"daily_fatalities" in its list, you should be okay.] First, save the plot to an object
...
```{r plotly1, fig.fullwidth = TRUE, fig.width = 8, fig.height = 3, caption = "Example of printing out a ggplot object by calling its assigned name."}
fatality_plot
```

To use plotly to make this plot interactive, all you need to do is run this `ggplot` object
through the `plotly` function. You can use the **pipe operator** (`%>%`) to do that: 

```{r plotly2, message = FALSE, fig.fullwidth = TRUE, fig.width = 8, fig.height = 3}
library(plotly)
fatality_plot %>% 
  ggplotly()
```
